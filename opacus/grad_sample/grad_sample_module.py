...
class GradSampleModule(nn.Module):
    ...
    def capture_backprops_hook(
        ...
        module.backprops = forward_output[0].detach()

        activations, backprops = self.rearrange_grad_samples(
            module, loss_reduction, batch_first
        )
        grad_sampler_fn = self.GRAD_SAMPLERS[type(module)]
        grad_sampler_fn(module, activations, backprops)

        if (not isinstance(activations, list) or len(activations) == 0) and hasattr(
            module, "max_batch_len"
        ):
            del module.max_batch_len

    def rearrange_grad_samples(
        self, module: nn.Module, loss_reduction: str, batch_first: bool
    ...
    def is_supported(cls, module: nn.Module) -> bool:
        ...
        return type(module) in cls.GRAD_SAMPLERS
