...
class DPLSTM(nn.Module):
    ...
    def _forward_cell(self, x: torch.Tensor, h_prev: torch.Tensor, c_prev: torch.Tensor):
    ...
    def forward(
        ...
        if state_init:
            ...
            h_init = torch.zeros(self.num_layers, batch_sz, self.hidden_size, dtype=x[0].dtype, device=x[0].device)
            c_init = torch.zeros(self.num_layers, batch_sz, self.hidden_size, dtype=x[0].dtype, device=x[0].device)
