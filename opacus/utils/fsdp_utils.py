...
def FSDP2Wrapper(model: nn.Module) -> nn.Module:
    sampler_classes = list(
        GradSampleModuleFastGradientClippingFSDP.GRAD_SAMPLERS.keys()
    ) + list(GradSampleModuleFastGradientClippingFSDP.NORM_SAMPLERS.keys())
    for module in iterate_submodules(model):
        module_type = next(
            (i for i in sampler_classes if isinstance(module, i)),
            type(module),
        )
        if (module_type in sampler_classes) or (not has_trainable_params(module)):
            fully_shard(module)
